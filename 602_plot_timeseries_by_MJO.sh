...
for (( j=0 ; j < $(( ${#region_params[@]} / $region_nparams )) ; j++ )); do
    ...
    output_dir=$fig_dir/fig_error_ts_Eabsmean_by_strictMJO-$days_per_window/$region_name
    mkdir -p $output_dir
    ...
    for categories in "NonMJO" "Ambiguous" "P1234" "P5678" ; do
        ...
        output=$output_dir/${ECCC_varset}-${varname}${level_str}_${category_str}.${fig_fmt}

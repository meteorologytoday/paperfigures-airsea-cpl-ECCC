...
for (( k=0 ; k < $(( ${#classify_params[@]} / $classify_nparams )) ; k++ )); do
    

    classify_method="${classify_params[$(( k * $nparams + 0 ))]}"


    python3 src/make_${classify_method}_category.py --no-display --output-root $gendata_dir


    category_file=$gendata_dir/${classify_method}_category.csv
    date_to_category_file=$gendata_dir/${classify_method}_date_to_category.csv
    output_root="$gendata_dir/analysis/output_analysis_map_by_${classify_method}_window-${days_per_window}-leadwindow-${lead_windows}"

...
#model_versions = ["GEPS6sub1offset1", "GEPS6sub1", "GEPS5",]# "GEPS6sub2", "GEPS6"]
...
def doJob(job_detail, detect_phase = False):
    ...
    try: 
        ...
        offset_days   = job_detail['offset_days']
        ...
        for dt in test_dts:
            ...
            if model_version == "GEPS6sub1offset1":
                test_model_version = "GEPS6sub1"
            else:
                test_model_version = model_version
             
            # Let start_time = Jan 2. If offset_days = 1, it means 
            # I am taking the second lead day, Jan 3, as the first day in the window
            # therefore, when dt = Jan 3, it is effectively asking the hindcast which
            # starts `offset_days` days earlier, which is Jan 2.
            test_start_time = dt - pd.Timedelta(days=offset_days)
            model_version_date = ECCC_tools.modelVersionReforecastDateToModelVersionDate(
                test_model_version,
                test_start_time,
            )
            ...
            start_times.append(test_start_time)
        ...
        if variable3D:
            ...
            dim_REG = (1, args.lead_windows, nlev, len(regions),)
        else:
            dim_E    = (1, args.lead_windows, aux_ds.dims["latitude"], aux_ds.dims["longitude"])
            dim_REG = (1, args.lead_windows, len(regions),)
        ...
        REGVARmean      = np.zeros(dim_REG)
        REGVAR2mean     = np.zeros(dim_REG)
        REGMEANVARmean  = np.zeros(dim_REG)
        REGMEANVAR2mean = np.zeros(dim_REG)
        REGPATTVARmean  = np.zeros(dim_REG)
        REGPATTVAR2mean = np.zeros(dim_REG)
        #REGREFmean      = np.zeros(dim_REG)
        #REGREF2mean     = np.zeros(dim_REG)
        #REGTOTmean      = np.zeros(dim_REG)
        #REGTOT2mean     = np.zeros(dim_REG)
        ...
        if ERA5_freq == "daily_mean": 
            ...
            # 2025-03-31
            # There should have been no adjustment in time for inst variables.
            # However, I made a mistake in downloading data code, which accidentally
            # add 24 hours to the lead time. Therefore, here is an ad-hoc correction.
        ...
        for k, start_time in enumerate(start_times):
            ...
            for p in range(args.lead_windows):
                ...
                _ds_ECCC = ds_ECCC[ECCC_varname].isel(lead_time=slice(offset_days + days_per_window*p, offset_days + days_per_window*(p+1)))

                if do_level_sel:
                    _ds_ECCC = _ds_ECCC.sel(level=args.levels)

                # Construct mean ERA5 data
                ref_data = None
                for l, lead_time in enumerate(_ds_ECCC.coords["lead_time"].to_numpy()):
                   
                    _ref_data = ERA5_loader.open_dataset_ERA5(
                    ...
                    if ERA5_varname_long in [ "mean_surface_sensible_heat_flux", "mean_surface_latent_heat_flux" ]:
                        ...
                        _ref_data *= reverse_sign

                    if do_level_sel:
                        #print(ref_data.coords)
                        _ref_data = _ref_data.sel(level=args.levels)
                ...
                ref_data /= len(_ds_ECCC.coords["lead_time"])

                lat = ref_data.coords["latitude"]
                lon = ref_data.coords["longitude"] % 360.0
                wgt = xr.apply_ufunc(np.cos, lat * np.pi/180.0)
                
                _ds_ECCC_mean = _ds_ECCC.mean(dim="lead_time")
                ...
                for number in _ds_ECCC_mean.coords["number"]:
                    
                    fcst_data = _ds_ECCC_mean.sel(number=number)
                        
                    fcst_error = fcst_data - ref_data
                    fcst_error_np = fcst_error.to_numpy()
                        
                    Emean[0, p]     += fcst_error_np
                    E2mean[0, p]    += fcst_error_np**2
                    #Eabsmean[0, p]  += np.abs(fcst_error_np)
                    #Eabs2mean[0, p] += np.abs(fcst_error_np)**2
                    total_cnt[0, p] += 1
                    ...
                    for r, region in enumerate(regions):
                        ...
                        if region not in region_idx_dict:    
                            ...
                            region_idx_dict[region] = region_idx

                        region_idx = region_idx_dict[region]

                        #region_fcst_data  = fcst_data.where(region_idx)
                        region_fcst_error = fcst_error.where(region_idx)
                        #region_ref_data   = ref_data.where(region_idx)

                        #mean_region_ref_data   = region_ref_data.weighted(wgt).mean(dim=["latitude", "longitude"], skipna=True)
                        #mean_region_fcst_data  = region_fcst_data.weighted(wgt).mean(dim=["latitude", "longitude"], skipna=True)
                        total_region_fcst_error = ((region_fcst_error**2.0).weighted(wgt).mean(dim=["latitude", "longitude"], skipna=True))**0.5
                        mean_region_fcst_error = region_fcst_error.weighted(wgt).mean(dim=["latitude", "longitude"], skipna=True)

                        anom_region_fcst_error = region_fcst_error - mean_region_fcst_error
                        anom_region_fcst_error = anom_region_fcst_error**2
                        anom_region_fcst_error = anom_region_fcst_error.weighted(wgt).mean(dim=["latitude", "longitude"], skipna=True)
                        anom_region_fcst_error = anom_region_fcst_error**0.5
                    

                        selector = None
                        if variable3D:
                            selector = (0, p, slice(None, None), r)
                        else:
                            selector = (0, p, r)

                        _REGVAR = total_region_fcst_error.to_numpy()**2.0 
                        _REGMEANVAR = mean_region_fcst_error.to_numpy()**2.0 
                        _REGPATTVAR = anom_region_fcst_error.to_numpy()**2.0 

                        REGMEANVARmean[selector] += _REGMEANVAR
                        REGPATTVARmean[selector] += _REGPATTVAR
 
                        REGMEANVAR2mean[selector] += _REGMEANVAR**2.0
                        REGPATTVAR2mean[selector] += _REGPATTVAR**2.0


 
                        REGVARmean[selector]  += _REGVAR
                        REGVAR2mean[selector] += _REGVAR**2.0

                        residue = _REGVAR - (_REGMEANVAR + _REGPATTVAR)
                        residue_frac = np.abs(residue / _REGVAR)
                        if np.any(residue_frac > 1e-3):
                            print("_REGVAR = ", _REGVAR)
                            print("_REGMEANVAR = ", _REGMEANVAR)
                            print("_REGPATTVAR = ", _REGPATTVAR)
                            print("Residue: ", residue)
                            raise Exception("Error: Residue of variance decomp larger than 1e-5")
                        #print("_REGVAR = ", _REGVAR)
                        #print("_REGMEANVAR = ", _REGMEANVAR)
                        #print("_REGPATTVAR = ", _REGPATTVAR)
                        #print("Residue: ", _REGVAR - (_REGMEANVAR + _REGPATTVAR))

                        #REGREFmean[selector]  += mean_region_ref_data.to_numpy()
                        #REGREF2mean[selector] += (mean_region_ref_data**2).to_numpy()
                        #REGTOTmean[selector]  += mean_region_fcst_data.to_numpy()
                        #REGTOT2mean[selector] += (mean_region_fcst_data**2).to_numpy()
        ...
        if variable3D:
            E_ttl_cnt_ext    = total_cnt[:, :, None, None, None]
            REG_ttl_cnt_ext = total_cnt[:, :, None, None]
        else:
            E_ttl_cnt_ext    = total_cnt[:, :, None, None]
            REG_ttl_cnt_ext = total_cnt[:, :, None]
            
        Emean     /= E_ttl_cnt_ext
        E2mean    /= E_ttl_cnt_ext
        #Eabsmean  /= E_ttl_cnt_ext
        #Eabs2mean /= E_ttl_cnt_ext

        REGMEANVARmean /= REG_ttl_cnt_ext
        REGPATTVARmean /= REG_ttl_cnt_ext
        REGMEANVAR2mean /= REG_ttl_cnt_ext
        REGPATTVAR2mean /= REG_ttl_cnt_ext
        
        REGVARmean  /= REG_ttl_cnt_ext
        REGVAR2mean /= REG_ttl_cnt_ext
 
        #REGREFmean   /= REG_ttl_cnt_ext
        #REGREF2mean  /= REG_ttl_cnt_ext
        #REGTOTmean   /= REG_ttl_cnt_ext
        #REGTOT2mean  /= REG_ttl_cnt_ext
        ...
        if variable3D:
            ...
            dim_REG   = [args.category_name, "lead_window", "level", "region"] 
            if args.levels is None:
            ...
            dim_REG = [args.category_name, "lead_window", "region"] 
        ...
        _tmp["%s_REGMEANVARmean" % ECCC_varname]   = (dim_REG, REGMEANVARmean)
        _tmp["%s_REGPATTVARmean" % ECCC_varname]   = (dim_REG, REGPATTVARmean)
        _tmp["%s_REGMEANVAR2mean" % ECCC_varname]  = (dim_REG, REGMEANVAR2mean)
        _tmp["%s_REGPATTVAR2mean" % ECCC_varname]  = (dim_REG, REGPATTVAR2mean)
        _tmp["%s_REGVARmean" % ECCC_varname]       = (dim_REG, REGVARmean)
        _tmp["%s_REGVAR2mean" % ECCC_varname]      = (dim_REG, REGVAR2mean)
        #_tmp["%s_REGREFmean" % ECCC_varname]       = (dim_REG, REGREFmean)
        #_tmp["%s_REGREF2mean" % ECCC_varname]      = (dim_REG, REGREF2mean)
        #_tmp["%s_REGTOTmean" % ECCC_varname]       = (dim_REG, REGTOTmean)
        #_tmp["%s_REGTOT2mean" % ECCC_varname]      = (dim_REG, REGTOT2mean)

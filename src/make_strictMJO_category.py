...
parser.add_argument('--hist-output', type=str, default="MJO_MAG_HIST_MJO,P1234,P5678.svg")
...
mjo_datafile = Path("data/omi.era5.1x.webpage.4023.txt.csv")
mjo_date_to_category_file = output_root / "strictMJO_date_to_category.csv"
mjo_category_file = output_root / "strictMJO_category.csv"
...
#categories = ["NonMJO", "P1278", "P3456", "Ambiguous"]
...
for i, row in df_mjo_data.iterrows():
    ...
    if start_phase != "Ambiguous":
        ...
        frac = np.sum(next_N['coarse_phase'].eq(start_phase)) / len(next_N['coarse_phase'])
        above_threshold = frac > 0.5
...
for i, row in df_MJO_category.iterrows():
    ...
    count[category] = len(df_mjo_data[df_mjo_data['category'] == category])
    
    print("[%d] `%s` => %d " % (i+1, category, len(df_mjo_data[df_mjo_data['category'] == category])))
...
for i, category in enumerate(hists.keys()):
    hist = hists[category]
    ax.bar(bin_centers, hist, width=bin_edges[1:] - bin_edges[:-1], bottom=bottom, label="%s (%d)" % (category, count[category]), color=colors[i])
    bottom += hist

ax.legend()
ax.set_xlabel("MJO Magnitude")
ax.set_ylabel("Frequency")
ax.set_title("Histogram of MJO Magnitude (total %d cases)" % (N_total,))
